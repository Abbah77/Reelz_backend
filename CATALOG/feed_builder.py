...
Each section is fetched concurrently. Results are cached at the whole-feed
level (1 hour) so repeated hits are instant.

Section IDs are stable strings the app can use for lazy-load calls.
"""
...
def _make_section(section_id: str, title: str, raw: list[dict], media_type: str, page: int) -> dict:
    items = [normalise_card(r, media_type) for r in raw[:20]]
    for item in items:
        item["section_tag"] = section_id
    has_more = len(raw) >= 20
    return {
        "id": section_id,
        "title": title,
...
async def build_feed(force: bool = False) -> dict:
    ...
    # Fan-out all section fetches concurrently
    (
    ...
    sections = [
        ...
        if s["items"]   # skip empty sections
    ]
...
# ── Section pagination (lazy load / pull-more) ─────────────────────────────────
...
async def get_section(section_id: str, cursor: Optional[str] = None, limit: int = 20) -> dict:
    if section_id not in _SECTION_FETCHERS:
        return {"items": [], "has_more": False, "next_cursor": None, "total_count": None}
